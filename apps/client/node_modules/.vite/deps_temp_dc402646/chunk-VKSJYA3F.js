...
var isEmpty2 = (x) => isNil(x) || isString(x) && !x || x instanceof Array && x.length === 0 || isObject2(x) && Object.keys(x).length === 0;
...
var mergeable = (left, right) => isObject2(left) && isObject2(right) || isArray4(left) && isArray4(right);
...
function walk(obj, selector, fn, options) {
  ...
  const next = names.slice(1).join(".");
  if (names.length === 1) {
    if (isObject2(obj) || isArray4(obj) && NUMBER_RE.test(key)) {
      fn(obj, key);
    }
  } else {
...
var Action2 = ((Action22) => {
  Action22[Action22["MAP"] = 0] = "MAP";
  Action22[Action22["FILTER"] = 1] = "FILTER";
  Action22[Action22["TAKE"] = 2] = "TAKE";
  Action22[Action22["DROP"] = 3] = "DROP";
  return Action22;
})(Action2 || {});
...
var $sort = (collection, sortKeys, options) => {
  if (isEmpty2(sortKeys) || !isObject2(sortKeys))
    return collection;
  let cmp = compare;
...
function $all(values, queries, options) {
  if (!isArray4(values) || !isArray4(queries) || !values.length || !queries.length) {
    return false;
  }
  let matched = true;
...
var bitwise = (op, compute) => (obj, expr, options) => {
  assert(isArray4(expr), `${op}: expression must be an array.`);
...
function processObject(obj, expr, options, expressionKeys, idOnlyExcluded) {
  ...
  for (const key of expressionKeys) {
    ...
    if (key === options.idKey && isEmpty2(subExpr)) {
      value = obj[key];
...
var $or = (_, rhs, options) => {
  assert(isArray4(rhs), "Invalid expression. $or expects value to be an Array");
  const queries = rhs.map((expr) => new Query(expr, options));
...
var wasLast = (input) => {
  const last3 = lastOfArray2(input.previousResults);
  if (last3 && last3[input.queryParams.primaryKey] === input.changeEvent.id) {
    return true;
  } else {
    return false;
  }
...
var wasSortedAfterLast = (input) => {
  ...
  if (last3[input.queryParams.primaryKey] === input.changeEvent.id) {
    return true;
  }
  const comp = input.queryParams.sortComparator(prev, last3);
  return comp > 0;
};
...
var isSortedAfterLast = (input) => {
  ...
  const comp = input.queryParams.sortComparator(doc, last3);
  return comp > 0;
};
...
var removeLastItem = (input) => {
  const last3 = input.previousResults.pop();
  if (input.keyDocumentMap && last3) {
    input.keyDocumentMap.delete(last3[input.queryParams.primaryKey]);
  }
};
...
//# sourceMappingURL=chunk-VKSJYA3F.js.map
